...
import {
  Code, ClipboardList, FileText, BookOpen, Clock, Calendar, Award,
  ...
  Sparkles, Layers, ArrowRight, Trophy, HelpCircle
} from 'lucide-react';
...
export default function TechnicalSession() {
  ...
  const fetchSessionData = async () => {
    ...
    try {
      ...
      if (assRes.status === 'fulfilled' && Array.isArray(assRes.value.data)) {
        setAssignments(assRes.value.data.filter(item => !isSoftSkillItem(item)));
      } else {
        setAssignments([]);
      }

      if (evalRes.status === 'fulfilled' && Array.isArray(evalRes.value.data)) {
        setAssessments(evalRes.value.data.filter(item => !isSoftSkillItem(item)));
      } else {
        setAssessments([]);
      }

      if (matRes.status === 'fulfilled' && Array.isArray(matRes.value.data)) {
        setMaterials(matRes.value.data.filter(item => !isSoftSkillItem(item)));
      } else {
        setMaterials([]);
      }
    } catch (err) {
      console.error('Error fetching technical session data:', err);
      toast.error('Unable to load technical session content.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const navigateToView = (mode) => {
    ...
    setStatusFilter('ALL');
    setViewMode(mode);
    if (mode === 'LANDING') navigate('/technical-session');
    else if (mode === 'ASSIGNMENTS') navigate('/technical-session/assignments');
    else if (mode === 'ASSESSMENTS') navigate('/technical-session/assessments');
    else if (mode === 'MATERIALS') navigate('/technical-session/study-materials');
  };
  ...
  const handleStudentSubmitAnswers = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        studentId: studentId,
        submissionRemarks: Object.values(studentAnswers).join(' | ') || 'Completed test answers.'
      };
  ...
  return (
    <div className="flex flex-col gap-8 font-sans pb-12">
      ...
      <div className="page-header flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-gradient-to-r from-blue-900 via-indigo-900 to-slate-900 text-white p-6 rounded-2xl shadow-xl">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="p-2 bg-blue-500/20 rounded-xl text-blue-300 border border-blue-400/30">
              <Code size={20} />
            </span>
            <h1 className="text-xl sm:text-2xl font-extrabold tracking-tight">Technical Session Portal</h1>
          </div>
          <p className="text-xs text-blue-200/80 max-w-xl">
            Access your technical learning resources, assignments, assessments and study materials.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 px-3.5 py-2 bg-white/10 backdrop-blur-md rounded-xl text-xs font-bold border border-white/20 select-none">
          <span className="text-blue-200 font-medium">Batch:</span>
          <span className="font-mono text-amber-300 bg-amber-400/20 px-2 py-0.5 rounded border border-amber-300/30">{batchId}</span>
          <span className="text-[10px] text-blue-200 uppercase font-semibold bg-white/10 px-1.5 py-0.5 rounded">READ ONLY</span>
        </div>
      </div>
      ...
      {loading ? (
        <div className="card p-16 text-center">
          ...
          <p className="text-xs text-gray-400 mt-3 font-semibold">Loading technical session content...</p>
        </div>
      ) : (
        <>
          {/* ========================================================================= */}
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div
                  ...
                  className="group card p-6 cursor-pointer hover:shadow-xl hover:border-blue-500 transition-all duration-300 flex flex-col justify-between border-t-4 border-t-blue-600 space-y-4"
                >
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${
                        totalAssCount > 0 ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-500'
                      }`}>
                        {totalAssCount} Tasks
                      </span>
                    </div>
                    ...
                    <div>
                      <h3 className="text-lg font-extrabold text-gray-900 group-hover:text-blue-600 transition">Assignments</h3>
                      <p className="text-xs text-gray-500 mt-1 leading-relaxed">
                        {totalAssCount > 0
                          ? 'View and submit your technical coding assignments and REST API projects.'
                ...
                <div
                  ...
                  className="group card p-6 cursor-pointer hover:shadow-xl hover:border-purple-500 transition-all duration-300 flex flex-col justify-between border-t-4 border-t-purple-600 space-y-4"
                >
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${
                        totalEvalCount > 0 ? 'bg-purple-100 text-purple-800' : 'bg-gray-100 text-gray-500'
                      }`}>
                    ...
                    <div>
                      <h3 className="text-lg font-extrabold text-gray-900 group-hover:text-purple-600 transition">Assessments</h3>
                      <p className="text-xs text-gray-500 mt-1 leading-relaxed">
                        {totalEvalCount > 0
                          ? 'Take scheduled monthly technical assessments and review your test results.'
                          : 'No assessments scheduled yet.'}
                      </p>
                    </div>
                  </div>

                  <div className="pt-4 border-t border-gray-100 flex items-center justify-between text-xs font-bold text-purple-600 group-hover:text-purple-700">
                ...
                >
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-1 leading-relaxed">
                        {materials.length > 0
                          ? 'Access programming reference PDFs, guides, and lecture slides.'
                ...
                <div className="card p-12 text-center bg-gray-50/60 border border-dashed border-gray-200 space-y-4 my-4">
                  ...
                  <div className="space-y-1 max-w-md mx-auto">
                    ...
                    <p className="text-xs text-gray-500 leading-relaxed">
                      There are currently no technical learning resources assigned to your batch (<span className="font-mono font-bold text-blue-700">{batchId}</span>).
                      Your trainer will publish assignments, assessments, and study materials here when they become available.
                    </p>
                  </div>
          ...
          {viewMode === 'ASSIGNMENTS' && (
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-between">
                ...
                >
                  <ArrowLeft size={14} />
                  <span>Back to Technical Session</span>
                </button>
                <span className="badge-blue text-xs font-bold">{totalAssCount} Total Tasks</span>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                ...
                <div className="card p-3.5 flex flex-col justify-between border-l-4 border-l-blue-600 shadow-2xs">
                  ...
                  <p className="text-xl font-extrabold text-blue-900 mt-1">{submittedAssCount}</p>
                </div>
              ...
              {filteredAssignments.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                  {filteredAssignments.map((item) => (
                    <div
                      key={item.id}
                      className="card p-5 hover:shadow-lg transition flex flex-col justify-between border-t-4 border-t-blue-600 space-y-4"
                    >
                      <div className="space-y-2.5">
                        <div className="flex items-center justify-between">
                          <span className="badge-blue text-[10px] font-bold">{item.subject || 'Technical'}</span>
                        ...
                        <p className="text-xs text-gray-500 line-clamp-2 leading-relaxed">{item.description || 'Assignment instructions.'}</p>
                      </div>

                      <div className="pt-3 border-t border-gray-100 space-y-2 text-[11px] font-medium text-gray-600">
                        <div className="flex items-center justify-between">
                          <span className="text-gray-400">Trainer:</span>
                          <span className="font-bold text-gray-800">{item.trainerName || 'Technical Trainer'}</span>
                        </div>
                        <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className="w-full btn bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs py-2 px-3 rounded-xl shadow-xs flex items-center justify-center gap-1.5 transition"
                      >
                        <FileText size={14} />
                ...
                <div className="card p-12 text-center bg-gray-50/50 border border-dashed border-gray-200">
                  ...
                  <p className="text-xs text-gray-500 mt-1 max-w-sm mx-auto leading-relaxed">
                    There are currently no assignments assigned to your batch for this session.
                  </p>
                </div>
              )}
            </div>
          ...
          {viewMode === 'ASSESSMENTS' && (
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="badge-purple text-xs font-bold">{totalEvalCount} Total Exams</span>
              </div>

              {/* Assessment Metrics */}
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div className="card p-4 flex flex-col justify-between border-l-4 border-l-purple-600 shadow-2xs">
                  <span className="text-[10px] font-bold text-purple-700 uppercase tracking-wide">Total Assessments</span>
                  <p className="text-2xl font-extrabold text-purple-900 mt-1">{totalEvalCount}</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                  {filteredAssessments.map((item) => {
                    ...
                    return (
                      <div
                        key={item.id}
                        className="card p-5 hover:shadow-lg transition flex flex-col justify-between border-t-4 border-t-purple-600 space-y-4"
                      >
                        <div className="space-y-2.5">
                          <div className="flex items-center justify-between">
                            <span className="badge-blue text-[10px] font-bold">{item.subject || 'DSA / Tech'}</span>
                            {compStatus === 'ONGOING' && (
                          ...
                          <h3 className="text-sm font-extrabold text-gray-900 line-clamp-1">{item.title}</h3>
                          <p className="text-xs text-gray-500 line-clamp-2 leading-relaxed">{item.description || 'Monthly assessment instructions.'}</p>
                        </div>

                        <div className="pt-3 border-t border-gray-100 space-y-2 text-[11px] font-medium text-gray-600">
                          <div className="flex items-center justify-between">
                            <span className="text-gray-400">Trainer:</span>
                            <span className="font-bold text-gray-800">{item.trainerName || 'Academic Trainer'}</span>
                          </div>
                          <div className="flex items-center justify-between">
                            <span className="text-gray-400">Batch ID:</span>
                            <span className="font-mono font-bold text-red-700">{item.batchId || batchId}</span>
            ...
            <div className="space-y-6">
              ...
              <div className="card p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="relative flex-1 min-w-[240px]">
                  ...
                  <input
                    type="text"
                    placeholder="Search technical materials by title or subject..."
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                    className="form-input pl-9 text-xs"
                  />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                  {filteredMaterials.map((item) => (
                    ...
                    >
                      <div className="space-y-2.5">
                        <div className="flex items-center justify-between">
                          <span className="badge-blue text-[10px] font-bold">{item.subject || 'Reference'}</span>
                        ...
                        <h3 className="text-sm font-extrabold text-gray-900 line-clamp-1">{item.title}</h3>
                        <p className="text-xs text-gray-500 line-clamp-2 leading-relaxed">{item.description || 'Technical study guide material.'}</p>
                      </div>

                      <div className="pt-3 border-t border-gray-100 space-y-2 text-[11px] font-medium text-gray-600">
                        <div className="flex items-center justify-between">
                          <span className="text-gray-400">Trainer:</span>
                          <span className="font-bold text-gray-800">{item.trainerName || 'Course Instructor'}</span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-gray-400">Batch ID:</span>
                          <span className="font-mono font-bold text-red-700">{item.batchId || batchId}</span>
        ...
        <div className="modal-backdrop">
          <div className="modal max-w-2xl">
            <div className="modal-header bg-gradient-to-r from-blue-50 to-indigo-50/60 pb-4 border-b border-blue-100">
              <div>
                <span className="badge-blue text-[10px] font-bold mb-1 inline-block">{selectedAssignment.subject || 'Technical'}</span>
            ...
            <div className="modal-body space-y-6">
              ...
              <div className="space-y-4">
                ...
                {(selectedAssignment.questions && selectedAssignment.questions.length > 0
                  ...
                  : [`Q1. Explain primary architecture principles and solution implementation for ${selectedAssignment.title}?`]
                ).map((qText, qIdx) => (
                    ...
                    {(selectedAssignment.submissionType === 'TEXT' || selectedAssignment.submissionType === 'TEXT_AND_FILE' || true) && (
                      <div className="pt-2">
                        ...
                        <textarea
                          ...
                          placeholder="Write your text answer or solution notes here..."
              ...
              {selectedAssignment.status !== 'EVALUATED' && (
                <form onSubmit={handleStudentUploadSubmission} className="space-y-4">
                    ...
                    {(selectedAssignment.submissionType === 'FILE' || selectedAssignment.submissionType === 'TEXT_AND_FILE' || selectedAssignment.submissionType !== 'NONE') && (
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-3">
                          <label className="btn bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs py-2 px-4 rounded-xl cursor-pointer inline-flex items-center gap-2">
                            <Upload size={14} />
                  ...
                  <div className="flex items-center justify-end gap-3">
                    ...
                    <button type="submit" disabled={uploading} className="btn-primary bg-blue-600 font-bold text-xs py-2.5 px-5">
        ...
        <div className="modal-backdrop">
          <div className="modal max-w-xl">
            <div className="modal-header bg-purple-50">
              <div>
                <h3 className="text-base font-bold text-purple-900">Take Assessment Examination</h3>
                <p className="text-xs text-purple-700">{selectedAssessment.title} • {selectedAssessment.totalMarks || 100} Marks</p>
              </div>
            ...
            <form onSubmit={handleStudentSubmitAnswers}>
              <div className="modal-body space-y-4">
                <div className="p-4 bg-purple-50/50 rounded-xl border border-purple-100 space-y-3">
                  ...
                  <p className="text-xs text-gray-700 leading-relaxed">
                    {selectedAssessment.description || `Explain key concepts, data structures, and architecture implementations for ${selectedAssessment.subject}?`}
                  </p>
                  <textarea
                    rows="4"
                    required
              ...
              <div className="modal-footer">
                ...
                <button type="submit" disabled={submittingAssessment} className="btn-primary bg-purple-600 font-bold">
      ...
      {viewResultModalOpen && selectedAssessment && (
        <div className="modal-backdrop">
          <div className="modal max-w-lg">
            ...
            <div className="modal-body space-y-5">
              <div className="bg-white p-5 rounded-2xl border border-emerald-200 shadow-2xs space-y-4">
                <div className="flex items-center justify-between border-b border-gray-100 pb-3">
                  <div>
                    <span className="badge-blue text-[10px] font-bold mb-1 inline-block">{selectedAssessment.subject || 'Technical'}</span>
                ...
                <div className="space-y-2 text-xs">
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold text-gray-800">{selectedAssessment.trainerName || 'Academic Trainer'}</span>
                  </div>
                  <div className="flex justify-between">
            ...
            <div className="modal-footer">
              <button onClick={() => setViewResultModalOpen(false)} className="btn-primary bg-emerald-600 font-bold">
                Close Result
              </button>
            </div>
          </div>
